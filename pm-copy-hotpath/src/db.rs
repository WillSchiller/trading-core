...
impl FillDb {
    ...
    pub async fn insert_fill(&self, fill: &FillRecord) -> Result<i64, HotPathError> {
        let client = self.pool.get().await.map_err(|e| HotPathError::Db(e.to_string()))?;
        ...
        let row = client.query_one(
            ...
            &[
                ...
                &fill.order_id,
                &fill.fill_price,
                &(fill.size_usd / fill.fill_price.max(0.01)),
    ...
    pub async fn get_trader_rolling_stats(&self, address: &str) -> Result<TraderRollingStats, HotPathError> {
        let client = self.pool.get().await.map_err(|e| HotPathError::Db(e.to_string()))?;
        let rows = client.query(
        ...
        let median_size = if sizes.is_empty() { 1.0 } else { sizes[sizes.len() / 2] };
        let total_trades = pnls.len();

        Ok(TraderRollingStats { pnls, total_trades, median_size })
    }
